...
function buildBookingEmailHtml(data, logoUrl) {
...
export default async function handler(req, res) {
  ...
  req.on("end", async () => {
    try {
      ...
      const logoUrl =
        process.env.BOOKING_LOGO_URL ||
        "https://cv.owellgraphics.com/assets/logo-B_KM6Ec3.png";

      const html = buildBookingEmailHtml(data, logoUrl);

      await transporter.sendMail({
        from: `"Windhoek CV Writers" <${process.env.SMTP_USER}>`,
